...
struct ChessSquare * 	get_square (ChessBoard chess_board, File file, Rank rank){
  if(file <= 'h' && file >= 'a' && rank <= 8 && rank > 0)
  {
...
{
  if(file <= 'h' && file >= 'a' && rank <= 8 && rank > 0 && is_square_occupied(chess_board, file, rank) == false)
  {
...
struct ChessPiece get_piece	(ChessBoard chess_board, File file, Rank rank)
{
  ...
  if(is_square_occupied(chess_board, file, rank) == false)
  {
    piece_to_return.type = NoPiece;
  }
  else
  {
    piece_to_return = chess_board[rank - 1][file - 'a'].piece;
  }
...
{
    if(chess_board[rank - 1][file - 'a'].is_occupied && validCoordinates(file, rank))
    {
...
{
  if(file <= 'h' && file >= 'a' && rank < 9 && rank > 0)
  {
    return true;
  }
  return false;
...
bool 	squares_share_file (File s1_f, Rank s1_r, File s2_f, Rank s2_r){
  ...
  if(s1_f == s2_f && validCoordinates(s1_f, s1_r) &&validCoordinates(s2_f, s2_r) )
  {
    return true;
  }
  return false;
